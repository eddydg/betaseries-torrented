...
(function() {
        ...
        "zooqle": {
            ...
            getLinks: (dom) => (
                [...dom.querySelectorAll('tbody tr')]
                    .map(tr => {
                        const healthStr = tr.children[5].children[0].title;
                        const [_, seedersStr, leechersStr] = healthStr.match(/Seeders: ([\d,]+) \| Leechers: ([\d,]+)/);
                        const seeders = parseInt(seedersStr.replace(',', '.'));
                        const leechers = parseInt(leechersStr);
    ...
    function getPertinentResults(query, results) {
        ...
        const queryWithAdditionalKeywords = query + " " + additionalKeywords.join(" ");
        const pertinentResults = fuse.search(queryWithAdditionalKeywords).map(r => r.item);
        console.info("Pertinent results", pertinentResults);
